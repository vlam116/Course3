...
## Now reading in the final txt files containing the numeric label for each of the 30 participants. 
...
replacement = c(names(features))

for(i in 1:length(replacement)) {
  names(x_bind)[i] = replacement[i]
}

y_bind = y_bind %>% rename(Activity = V1)
y_bind$Activity = as.factor(y_bind$Activity)
...
xy_bind = bind_cols(y_bind, x_bind)
